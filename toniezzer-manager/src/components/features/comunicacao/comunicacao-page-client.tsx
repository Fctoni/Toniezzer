...
interface ComunicacaoPageClientProps {
  initialTopicos: TopicoWithRelations[];
  etapas: Tables<"etapas">[];
}

export function ComunicacaoPageClient({
  initialTopicos,
  etapas,
}: ComunicacaoPageClientProps) {
  const { currentUser } = useCurrentUser();
  const [topicos, setTopicos] = useState<TopicoWithRelations[]>(initialTopicos);
  ...
  const [expandedEtapas, setExpandedEtapas] = useState<Set<string>>(
    new Set(["geral", ...etapas.map((e) => e.id)])
  );
  ...
  useEffect(() => {
    if (statusFilter !== "todos" || search) {
      fetchTopicos();
    }
  }, [fetchTopicos, statusFilter, search]);
