# Índice de Alterações

Este arquivo lista todas as alterações do projeto e seu status atual.

---

## Legenda de Status

| Status | Descrição |
|--------|-----------|
| 🟢 Finalizado | Alteração implementada e testada |
| 🟠 Aguardando testes | Implementação feita, aguardando testes manuais |
| 🔵 Pronto para executar | Spec criada, aguardando implementação |
| 🟡 Em discussão | Proposta em discussão ou planejamento |
| 🔴 Em execução | Implementação em andamento |
| ❌ Cancelado | Alteração cancelada |
| 📋 Referência | Documento de referência ou índice |

## Legenda - Incluso no PRD

| Status | Descrição |
|--------|-----------|
| ✅ concluído | Documentação adicionada ao PRD-FINAL.md |
| ⏳ pendente | Implementação feita, falta documentar no PRD |
| 🔸 parcial | Parcialmente documentado no PRD |
| ➖ não necessário | Não requer documentação no PRD (referências, índices, procedimentos) |

---

## Ativos

| # | Arquivo | Descrição | Status | PRD |
|---|---------|-----------|--------|-----|
| 03 | [spec-alteracao03.md](./spec-alteracao03.md) | Centralizar CRUD em services (Fase 1: Cronograma) | 🟠 Aguardando testes | ⏳ pendente |
| 04 | [spec-alteracao04.md](./spec-alteracao04.md) | Centralizar CRUD em services (Fase 2: Financeiro) | 🟠 Aguardando testes | ⏳ pendente |
| 09 | [spec-alteracao09.md](./spec-alteracao09.md) | Convenções: import order + rename de arquivo | 🔵 Pronto para executar | ⏳ pendente |
| 10 | [alteracao10.md](./alteracao/alteracao04.md) | Atualização otimista local em vez de router.refresh() | 🟡 Em discussão | ⏳ pendente |
| 11 | [spec-alteracao05.md](./spec/spec-alteracao05.md) | Editar data de vencimento de parcelas inline | 🔵 Pronto para executar | ⏳ pendente |
| 12 | [spec-alteracao06.md](./spec/spec-alteracao06.md) | Corpo do email e parcelas editaveis na pagina de emails | 🔵 Pronto para executar | ⏳ pendente |
| 13 | [spec-alteracao07.md](./spec/spec-alteracao07.md) | Salvar anexos de emails no Supabase Storage durante sincronizacao | 🔵 Pronto para executar | ⏳ pendente |

## PRD / Commit Pendente

Implementação finalizada, mas falta documentar no PRD ou commitar.

| # | Arquivo | Descrição | PRD |
|---|---------|-----------|-----|
| 01 | [spec-alteracao01.md](./spec-alteracao01.md) | Sistema de 3 níveis: Etapa → Subetapa → Tarefas | ⏳ pendente |
| 02 | [spec-alteracao02.md](./spec/spec-alteracao02.md) | Substituir kanban por tabela na página de emails | ⏳ pendente |
| 05 | [spec-alteracao05.md](./spec-alteracao05.md) | Centralizar CRUD em services (Fase 3: Restante) | ⏳ pendente |
| 06 | [spec-alteracao06.md](./spec-alteracao06.md) | Refatoração de padrões de código (pós-auditoria) | 🟢 Finalizado | ⏳ pendente |
| 07 | [spec-alteracao07.md](./spec-alteracao07.md) | Correções em services: error handling + return types | 🟢 Finalizado | ⏳ pendente |
| 08 | [spec-alteracao08.md](./spec-alteracao08.md) | API Routes: Zod + client imports + Server Components | ⏳ pendente |


## Finalizados

| # | Arquivo | Descrição | PRD |
|---|---------|-----------|-----|
